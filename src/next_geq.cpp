...
void perf_next_geq(char const* binary_filename,
    ...
    size_t total = 0;
    std::cout << "performing " << queries.size() << " next_geq queries..."
              << std::endl;
    ...
    for (int run = 0; run != runs; ++run) {
        ...
        for (uint32_t i = 0; i != index.size(); ++i) {
            ...
            for (uint32_t j = 0; j != num_queries_per_sequence; ++j) {
                total += sequence.next_geq(queries[q++]);
            }
        }
        t.stop();
    }
...
int main(int argc, char** argv) {
    ...
    perf_next_geq(binary_filename, num_queries_per_sequence);
